(function () {
  ...
  const expandedSignalBaseline = [
    { id: 5032, vendorId: "tcs", source: "Vendor Master", sourceName: "Vendor Master", title: "PwC India continuity coverage reaches target", summary: "Quarterly resource review shows named backup coverage for 37 of 40 critical advisory roles, improving from 82.5% to 92.5% against a 90.0% continuity target.", type: "People", signalType: "People", sentiment: "Positive", dimension: "R", confidence: 87, impact: 6, status: "pending", aiProvider: "rules", aiExplanation: "Measured backup coverage above target reduces key-person dependency and strengthens Risk Profile.", createdAt: "2026-06-20T09:30:00.000Z" },
    { id: 5031, vendorId: "infosys", source: "Internal SLA Review", sourceName: "Internal SLA Review", title: "EY India deliverable rework rises above tolerance", summary: "Quality review found 13 of 118 deliverables required material rework, an 11.0% rate versus the 6.0% tolerance and 7.2% in the previous quarter.", type: "Operational", signalType: "Operational", sentiment: "Negative", dimension: "P", confidence: 88, impact: 6, status: "pending", aiProvider: "rules", aiExplanation: "Rework rate above tolerance indicates measurable delivery-quality pressure in Performance.", createdAt: "2026-06-20T09:25:00.000Z" },
    { id: 5030, vendorId: "sabrelink", source: "LittleBig Mission Feed", sourceName: "LittleBig Mission Feed", title: "Deloitte India milestone acceptance improves", summary: "First-pass milestone acceptance reached 93.6% across 47 submitted milestones, up from 86.9%, while average acceptance time fell from 4.8 to 3.1 days.", type: "Operational", signalType: "Operational", sentiment: "Positive", dimension: "I", confidence: 91, impact: 7, status: "pending", aiProvider: "rules", aiExplanation: "Higher first-pass acceptance and faster handoffs strengthen Integration.", createdAt: "2026-06-20T09:20:00.000Z" },
    { id: 5029, vendorId: "globex", source: "Coupa Spend Export", sourceName: "Coupa Spend Export", title: "KPMG India unapproved spend exceeds control limit", summary: "Coupa review identified INR 28.4 lakh of spend outside approved statements of work, equal to 6.3% of quarterly spend versus the 2.0% control limit.", type: "Financial", signalType: "Financial", sentiment: "Negative", dimension: "S", confidence: 93, impact: 8, status: "pending", aiProvider: "rules", aiExplanation: "Quantified off-contract spend above the control limit weakens Score & Value and requires commercial review.", createdAt: "2026-06-20T09:15:00.000Z" },
    { id: 5028, vendorId: "cloud-engineering", source: "Approved Market Intelligence", sourceName: "Approved Market Intelligence", title: "Accenture India cloud certification coverage expands", summary: "Certified cloud specialists supporting the account increased from 64 to 79, lifting role coverage from 81% to 94% against the 90% capability target.", type: "People", signalType: "People", sentiment: "Positive", dimension: "M", confidence: 83, impact: 5, status: "pending", aiProvider: "rules", aiExplanation: "Certification coverage above the capability target strengthens Market Fit for cloud delivery.", createdAt: "2026-06-20T09:10:00.000Z" },
    { id: 5027, vendorId: "facilities", source: "Internal SLA Review", sourceName: "Internal SLA Review", title: "Capgemini India repeat incidents remain elevated", summary: "Repeat incidents represented 14.7% of 286 monthly tickets versus an 8.0% threshold; 19 repeats were linked to three unresolved root causes.", type: "Operational", signalType: "Operational", sentiment: "Negative", dimension: "I", confidence: 86, impact: 6, status: "pending", aiProvider: "rules", aiExplanation: "Repeat-incident concentration and unresolved root causes weaken service Integration.", createdAt: "2026-06-20T09:05:00.000Z" },
    { id: 5026, vendorId: "talent-marketplace", source: "Vendor Master", sourceName: "Vendor Master", title: "Wipro critical-role backup coverage below target", summary: "Only 21 of 29 critical engineering roles have validated backups, a 72.4% coverage rate versus the 90.0% resilience target.", type: "Risk", signalType: "Risk", sentiment: "Negative", dimension: "R", confidence: 89, impact: 7, status: "pending", aiProvider: "rules", aiExplanation: "Insufficient backup coverage creates measurable continuity exposure in Risk Profile.", createdAt: "2026-06-20T09:00:00.000Z" },
    { id: 5025, vendorId: "procurement-platform", source: "Coupa Spend Export", sourceName: "Coupa Spend Export", title: "Cognizant India early-payment capture improves", summary: "Finance controls captured 87% of eligible early-payment discounts across 214 invoices, up from 68%, realizing INR 9.6 lakh in quarterly savings.", type: "Financial", signalType: "Financial", sentiment: "Positive", dimension: "S", confidence: 90, impact: 6, status: "pending", aiProvider: "rules", aiExplanation: "Measured discount capture and realized savings strengthen Score & Value.", createdAt: "2026-06-20T08:55:00.000Z" }
  ];
  const existingSignalTitles = new Set(bundledSeedDatabase.signals.map((signal) => signal.title));
  bundledSeedDatabase.signals.push(...expandedSignalBaseline.filter((signal) => !existingSignalTitles.has(signal.title)));
    ...
    if (method === "POST" && path === "/api/internal-inputs") {
      ...
      const input = {
        ...
        attachment: body.attachment ? {
          ...
          category: body.attachment.category || "Supporting evidence",
          confidentiality: body.attachment.confidentiality || "Internal",
          owner: body.attachment.owner || "",
          expiryDate: body.attachment.expiryDate || null,
          storageMode: "browser"
        } : null,
      ...
      addAudit(db, `${vendor.name}: internal gate input applied to ${prismLabels[input.dimension]}${input.attachment ? ` with ${input.attachment.fileName} attached` : ""}.`, user.name, { inputId: input.id, vendorId: vendor.id, attachment: input.attachment ? { fileName: input.attachment.fileName, category: input.attachment.category } : null });
      saveDb(db);
